package de.furkan.voicescape.jsocksSocksProxy;
 ...
 * Can be used same way as the normal DatagramSocket. One should be carefull though with the
 * datagram sizes used, as additional data is present in both incomming and outgoing datagrams.
 *
 * <p>SOCKS5 protocol allows to send host address as either:
 *
 * <ul>
 *   <li>IPV4, normal 4 byte address. (10 bytes header size)
 *   <li>IPV6, version 6 ip address (not supported by Java as for now). 22 bytes header size.
 *   <li>Host name,(7+length of the host name bytes header size).
 * </ul>
 * <p>
 * As with other Socks equivalents, direct addresses are handled transparently, that is data will be
 * send directly when required by the proxy settings.
 *
 * <p><b>NOTE:</b><br>
 * Unlike other SOCKS Sockets, it <b>does not</b> support proxy chaining, and will throw an
 * exception if proxy has a chain proxy attached. The reason for that is not my laziness, but rather
 * the restrictions of the SOCKSv5 protocol. Basicaly SOCKSv5 proxy server, needs to know from which
 * host:port datagrams will be send for association, and returns address to which datagrams should
 * be send by the client, but it does not inform client from which host:port it is going to send
 * datagrams, in fact there is even no guarantee they will be send at all and from the same address
 * each time.
 */
public class Socks5DatagramSocket extends DatagramSocket {

    private final Logger log = LoggerFactory.getLogger(Socks5DatagramSocket.class);
    ...
    /**
     * Construct Datagram socket for communication over SOCKS5 proxy server. This constructor uses
     * default proxy, the one set with Proxy.setDefaultProxy() method. If default proxy is not set or
     * it is set to version4 proxy, which does not support datagram forwarding, throws SocksException.
     */
    ...
    /**
     * Construct Datagram socket for communication over SOCKS5 proxy server. And binds it to the
     * specified local port. This constructor uses default proxy, the one set with
     * Proxy.setDefaultProxy() method. If default proxy is not set or it is set to version4 proxy,
     * which does not support datagram forwarding, throws SocksException.
     */
    ...
    /**
     * Construct Datagram socket for communication over SOCKS5 proxy server. And binds it to the
     * specified local port and address. This constructor uses default proxy, the one set with
     * Proxy.setDefaultProxy() method. If default proxy is not set or it is set to version4 proxy,
     * which does not support datagram forwarding, throws SocksException.
     */
    ...
    /**
     * Constructs datagram socket for communication over specified proxy. And binds it to the given
     * local address and port. Address of null and port of 0, signify any availabale port/address.
     ...
     * </ol>
     * <p>
     * Might throw IOException if binding datagram socket to given address/port fails. See
     * java.net.DatagramSocket for more details.
     */
    public Socks5DatagramSocket(SocksProxyBase p, int port, InetAddress ip) throws IOException {
        super(port, ip);
    ...
    Socks5DatagramSocket(
            boolean server_mode, UDPEncapsulation encapsulation, InetAddress relayIP, int relayPort)
            throws IOException {
        super();
    ...
    /**
     * Receives udp packet. If packet have arrived from the proxy relay server, it is processed and
     * address and port of the packet are set to the address and port of sending host.<br>
     * If the packet arrived from anywhere else it is not changed.<br>
     * <B> NOTE: </B> DatagramPacket size should be at least 10 bytes bigger than the largest packet
     * you expect (this is for IPV4 addresses). For hostnames and IPV6 it is even more.
     *
    ...
    public void receive(DatagramPacket dp) throws IOException {
        ...
        final ByteArrayInputStream bIn = new ByteArrayInputStream(data, offset, dp.getLength());
    ...
    /**
     * Returns port assigned by the proxy, to which datagrams are relayed. It is not the same port to
     * which other party should send datagrams.
     *
     * @return Port assigned by socks server to which datagrams are send for association.
     */
    ...
    /**
     * Address assigned by the proxy, to which datagrams are send for relay. It is not necesseraly the
     * same address, to which other party should send datagrams.
     *
     ...
     *
     * <p>This methods checks wether the primary connection to proxy server is active. If it is,
     * chances are that proxy continues to forward datagrams being send from this socket. If it was
     * closed, most likely datagrams are no longer being forwarded by the server.
     *
     * <p>Proxy might decide to stop forwarding datagrams, in which case it should close primary
     * connection. This method allows to check, wether this have been done.
     *
     * <p>You can specify timeout for which we should be checking EOF condition on the primary
     * connection. Timeout is in milliseconds. Specifying 0 as timeout implies infinity, in which case
     * method will block, until connection to proxy is closed or an error happens, and then return
     * false.
     *
     * <p>One possible scenario is to call isProxyactive(0) in separate thread, and once it returned
     * notify other threads about this event.
     *
     * @param timeout For how long this method should block, before returning.
     * @return true if connection to proxy is active, false if eof or error condition have been
     * encountered on the connection.
     */
