...
public class Socks5Proxy extends SocksProxyBase implements Cloneable {
     ...
     *
     * <p>SOCKS5 protocol allows to send host names rather then IPs in the requests, this option
     * controls wether the hostnames should be send to the proxy server as names, or should they be
     * resolved locally.
     *
    ...
    public Authentication getAuthenticationMethod(int methodId) {
        final Object method = authMethods.get(Integer.valueOf(methodId));
        if (method == null) {
            return null;
        }
