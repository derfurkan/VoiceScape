...
/**
 * SocksSocket tryies to look very similar to normal Socket, while allowing connections through the
 * SOCKS4 or 5 proxy. To use this class you will have to identify proxy you need to use, Proxy class
 * allows you to set default proxy, which will be used by all Socks aware sockets. You can also
 * create either Socks4Proxy or Socks5Proxy, and use them by passing to the appropriate
 * constructors.
 *
 * <p>Using Socks package can be as easy as that:
 *
 * <pre>
 * &lt;tt&gt;
 *
 ...
 * </pre>
 *
 * <p>However if the need exist for more control, like resolving addresses remotely, or using some
 * non-trivial authentication schemes, it can be done.
 */
public class SocksSocket extends Socket {
    private final Logger log = LoggerFactory.getLogger(SocksSocket.class);
    // Data members
    ...
    /**
     * Tryies to connect to given host and port using default proxy. If no default proxy speciefied it
     * throws SocksException with error code SOCKS_NO_PROXY.
     *
     ...
     *                              <ol>
     *                                <li>Proxy settings say that address should be resolved locally, but this fails.
     *                                <li>Proxy settings say that the host should be contacted directly but host name can't be
     *                                    resolved.
     *                              </ol>
     * @throws SocksException       If one of the following happens:
     *                              <ul>
     *                                <li>Proxy is is null.
     *                                <li>Proxy settings say that the host should be contacted directly but this fails.
    ...
    /**
     * Tryies to connect to given ip and port using default proxy. If no default proxy speciefied it
     * throws SocksException with error code SOCKS_NO_PROXY.
     *
    ...
    /**
     * These 2 constructors are used by the SocksServerSocket. This socket simply overrides
    ...
    /**
     * Returns remote host name, it is usefull in cases when addresses are resolved by proxy, and we
     * can't create InetAddress object.
     *
    ...
    /**
     * Get remote host as InetAddress object, might return null if addresses are resolved by proxy,
     * and it is not possible to resolve it locally
     *
     * @return Ip address of the host this socket is connected to, or null if address was returned by
     * the proxy as DOMAINNAME and can't be resolved locally.
     */
    ...
    /**
     * Get address assigned by proxy to make a remote connection, it might be different from the host
     * specified for the proxy. Can return null if socks server returned this address as hostname and
     * it can't be resolved locally, use getLocalHost() then.
     *
    ...
    /**
     * Get name of the host, proxy has assigned to make a remote connection for this socket. This
     * method is usefull when proxy have returned address as hostname, and we can't resolve it on this
     * machine.
     *
