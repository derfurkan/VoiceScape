...
class Socks5Message extends ProxyMessage {
    ...
    private final Logger log = LoggerFactory.getLogger(Socks5Message.class);
    /**
     ...
     * @throws SocksException If server response code is not SOCKS_SUCCESS(0), or if any error with
     *                        protocol occurs.
     ...
     *
     * @param in If true read server response, else read client request.
     * @throws SocksException If server response code is not SOCKS_SUCCESS(0) and reading in client
     *                        mode, or if any error with protocol occurs.
     * @throws IOException    If any error happens with I/O.
     */
    ...
    /**
     * Wether to resolve hostIP returned from SOCKS server that is wether to create InetAddress object
    ...
    /**
     * Returns IP field of the message as IP, if the message was created with ATYP of HOSTNAME, it
     * will attempt to resolve the hostname, which might fail.
     *
