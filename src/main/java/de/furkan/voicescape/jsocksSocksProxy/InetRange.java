...
/**
 * Class InetRange provides the means of defining the range of inetaddresses. It's used by Proxy
 * class to store and look up addresses of machines, that should be contacted directly rather then
 * through the proxy.
 *
 * <p>InetRange provides several methods to add either standalone addresses, or ranges (e.g.
 * 100.200.300.0:100.200.300.255, which covers all addresses on on someones local network). It also
 * provides methods for checking wether given address is in this range. Any number of ranges and
 * standalone addresses can be added to the range.
 */
public class InetRange implements Cloneable {
     ...
     * This will be treated as range ddd.ddd.ddd.0 to ddd.ddd.ddd.255. It is not necessary to
     * specify 3 first bytes you can use just one or two. For example 130. will cover address
     * between 130.0.0.0 and 13.255.255.255.
    ...
    /**
     * Adds another range of ips for this range.Any host with ip address greater than or equal to the
     * address of from and smaller than or equal to the address of to will be included in the range.
     *
    ...
    public synchronized void add(final InetAddress from, final InetAddress to) {
        all.addElement(
                new Object[]{
                        from.getHostAddress() + ":" + to.getHostAddress(), null, ip2long(from), ip2long(to)
                });
    }
     ...
     *   <li>If attemptResolve is true, host is name, rather than ip, and all previous attempts
     *       failed, try to resolve the hostname, and check wether the ip associated with the host is
     *       in the range.It also repeats all previos steps with the hostname obtained from
     *       InetAddress, but the name is not allways the full name,it is quite likely to be the same.
     *       Well it was on my machine.
     * </ol>
     * <p>
     * Host name to check.
     *
     * @param attemptResolve Wether to lookup ip address which corresponds to the host,if required.
     * @return true If host is in the range, false otherwise.
     */
    public synchronized boolean contains(final String host0, final boolean attemptResolve) {
        if (all.size() == 0) {
    ...
    /**
     * Same as previous but used internally, to avoid unnecessary convertion of IPs, when checking
     * subranges
     */
    ...
    private void resolve(final Object[] entry, final String from, final String to) {
        long f, t;
